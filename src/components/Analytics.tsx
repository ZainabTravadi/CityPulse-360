...
const Analytics = () => {
  ...
  useEffect(() => {
    const fetchForecast = async () => {
      try {
        const res = await fetch("http://localhost:5000/electricity");
        const data = await res.json();

        const chartData = data.forecast.map((item: any) => ({
          time: item.timestamp, // ✅ keep timestamp
          value: item.value,
        }));
...
<Card className="card-space p-6">
  ...
  <p className="text-sm text-muted-foreground mb-6">
    Indicates the reliability of the model’s predictions based on validation
  ...
  <div className="space-y-2 text-center mb-4">
    ...
    <p className="text-xs text-muted-foreground">
      Based on 1,440 data points
    </p>
  </div>

  {/* ✅ Confidence Range Legend */}
  <div className="grid grid-cols-3 gap-2 text-center text-xs text-muted-foreground mb-4">
    <div>
      <div className="h-2 w-full bg-red-400 rounded"></div>
      Low
    </div>
    <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className="card-space p-6 h-full">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Explains which features (e.g., weather, time of day) most
