...
const KPICard = ({
  ...
  // 🔹 Hard-map colors to each trend type
  const getTrendColor = () => {
    switch (trend) {
      case "up":
        return "text-red-500"; // 🔴 Red for upward trend
      case "down":
        return "text-blue-500"; // 🔵 Blue for downward trend
      default:
        return "text-yellow-500"; // 🟡 Gold for neutral
    }
  };
    ...
    >
      <div className="flex items-start justify-between">
        <div className="space-y-2">
          ...
          {trend && trendValue && (
            <div
              className={cn("flex items-center space-x-1 text-xs font-semibold", getTrendColor())}
            >
              {getTrendIcon()}
